...
{
    partial class frm_Phongban : Form
    {
        public frm_Phongban()
        {
            InitializeComponent();
        ...
        private void Setup()
        {
            dgv.DataSource = db.phongbans.OrderBy(w => w.ten).ToList();
        }
        ...
        private void dgv_RowEnter(object sender, DataGridViewCellEventArgs e)
        {
            try
            {
                ...
                txtCap.Text = dgv.Rows[e.RowIndex].Cells["cap"].Value.ToString();
            }
            catch (Exception a)
            {
        ...
        private void btnXoa_Click(object sender, EventArgs e)
        {
            if (Functions.KiemtraQuyen(db, this.Name, DateTime.Now.Date, "xoa"))
            {
                ...
                if (dialogResult == DialogResult.Yes)
                {
                    ...
                    var query = db.phongbans.Where(w => w.ma == txtMa.Text).FirstOrDefault();
                    db.phongbans.DeleteOnSubmit(query);
        ...
        private bool Check_NoExist()
        {
            ...
            if (!sua)
            {
                var q = db.phongbans.Where(w => w.ma == txtMa.Text.Trim().ToLower()).FirstOrDefault();
                if (q != null)
                    return false;
            }
        ...
        private void Insert()
        {
            LINQ.phongban a = new LINQ.phongban();
            a.ma = txtMa.Text.Trim().ToLower();
            a.ten = txtTen.Text;
            a.cap = Functions.ChuyenInt16(txtCap.Text);


            db.phongbans.InsertOnSubmit(a);
            db.SubmitChanges();
        }
        private void UpdateData()
        {
            var a = db.phongbans.Where(w => w.ma == txtMa.Text.Trim()).FirstOrDefault();
            if (a != null)
            {
                a.ten = txtTen.Text;  
                a.cap = Functions.ChuyenInt16(txtCap.Text);
