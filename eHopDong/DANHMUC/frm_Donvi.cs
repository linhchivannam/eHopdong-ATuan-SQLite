...
namespace eHopdong.DANHMUC
{
    partial class frm_Donvi : Form
    {
        ...
        private void Setup()
        {
            dgv.DataSource = db.coquandonvis.Where(w=>w.ten.ToLower().Contains(txtSearch.Text.ToLower())).OrderBy(w => w.ten).ToList();
        }
        ...
        private void dgv_RowEnter(object sender, DataGridViewCellEventArgs e)
        {
            try
            {
                ...
                txtDC.Text = dgv.Rows[e.RowIndex].Cells["diachi"].Value.ToString();
                txtDT.Text = dgv.Rows[e.RowIndex].Cells["dienthoai"].Value.ToString();
                txtMST.Text = dgv.Rows[e.RowIndex].Cells["mst"].Value.ToString();
            }
            catch (Exception a)
            {
        ...
        private void btnXoa_Click(object sender, EventArgs e)
        {
            if (Functions.KiemtraQuyen(db, this.Name, DateTime.Now.Date, "xoa"))
            {
                ...
                if (dialogResult == DialogResult.Yes)
                {
                    ...
                    var query = db.coquandonvis.Where(w => w.ma ==Functions.ChuyenInt(txtMa.Text)).FirstOrDefault();
                    db.coquandonvis.DeleteOnSubmit(query);
        ...
        private bool Check_NoExist()
        {
            ...
            if (!sua)
            {
                var q = db.coquandonvis.Where(w => w.ma == Functions.ChuyenInt(txtMa.Text)).FirstOrDefault();
                if (q != null)
                    return false;
            }
        ...
        private void Insert()
        {
            LINQ.coquandonvi a = new LINQ.coquandonvi();
            ...
            db.coquandonvis.InsertOnSubmit(a);
            db.SubmitChanges();
        }
        private void UpdateData()
        {
            var a = db.coquandonvis.Where(w => w.ma ==Functions.ChuyenInt(txtMa.Text.Trim())).FirstOrDefault();
            if (a != null)
            {
                a.ten = txtTen.Text;
                a.diachi = txtDC.Text;
