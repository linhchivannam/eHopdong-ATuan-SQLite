...
{
    public partial class uBaoKHTiemnang : UserControl
    {
        private static uBaoKHTiemnang _instance;
        public static uBaoKHTiemnang Instance
        {
            get
            {
                if (_instance == null)
                    _instance = new uBaoKHTiemnang();

                return _instance;
            }
        }
        public uBaoKHTiemnang()
        {
            InitializeComponent();
        }
        ...
        public void LoadKhachhang()
        {
            ...
            if (PQ.Permission == "0")
            {
                ...
                var t = db.khachhangs.Where(w => w.tiemnang.Value==true & w.ten.ToLower().Contains(txtSearch.Text.ToLower())).OrderBy(w => w.ten).ToList().OrderBy(w=>w.tendonvi).ThenBy(w=>w.ten);
            ...
            {
                var t = db.DS_KHACHHANG_QUANLY(PQ.Bophan).AsEnumerable().Where(w => w.tiemnang.Value == true & w.ten.ToLower().Contains(txtSearch.Text.ToLower())).OrderBy(w => w.ten).ToList().OrderBy(w => w.tendonvi).ThenBy(w => w.ten);
                var t1 = chkAll.Checked ? t : t.Where(w => w.tinh == cbxTinh.SelectedValue.ToString());
            ...
            {
                var t = db.khachhangs.Where(w => w.tiemnang.Value == true & w.username==PQ.UserName & w.ten.ToLower().Contains(txtSearch.Text.ToLower())).OrderBy(w => w.ten).ToList().OrderBy(w => w.tendonvi).ThenBy(w => w.ten);               
