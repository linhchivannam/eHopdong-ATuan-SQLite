...
{
    partial class frm_DlgHopdongDonvi : Form
    {
        public frm_DlgHopdongDonvi()
        {
            InitializeComponent();
        }
        public long idkh;
...
private void frm_DlgHopdongcanhan_Load(object sender, EventArgs e)
        ...
        private void LoadData()
        {
            ...
            if (PQ.Permission == "0")
            {
                var t = db.DS_DICHVU_MADONVI(idkh).Where(w =>  (w.noidung.ToLower().Contains(txtSearch.Text.ToLower()))).OrderByDescending(w => w.ngay).ThenByDescending(w => w.id).ToList();
