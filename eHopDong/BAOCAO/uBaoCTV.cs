    ...
    public partial class uBaoCTV : UserControl
    {
        ...
        public void LoadKhachhang()
        {
            ...
            if (PQ.Permission == "0")
            {
                ...
                var t = db.khachhangs.Where(w => w.congtacvien.Value==true & w.ten.ToLower().Contains(txtSearch.Text.ToLower())).OrderBy(w => w.ten).ToList().OrderBy(w=>w.tendonvi).ThenBy(w=>w.ten);
            ...
            {
                var t = db.DS_KHACHHANG_QUANLY(PQ.Bophan).AsEnumerable().Where(w => w.congtacvien.Value == true & w.ten.ToLower().Contains(txtSearch.Text.ToLower())).OrderBy(w => w.ten).ToList().OrderBy(w => w.tendonvi).ThenBy(w => w.ten);
                var t1 = chkAll.Checked ? t : t.Where(w => w.tinh == cbxTinh.SelectedValue.ToString());
            ...
            {
                var t = db.khachhangs.Where(w => w.congtacvien.Value == true & w.username==PQ.UserName & w.ten.ToLower().Contains(txtSearch.Text.ToLower())).OrderBy(w => w.ten).ToList().OrderBy(w => w.tendonvi).ThenBy(w => w.ten);               
